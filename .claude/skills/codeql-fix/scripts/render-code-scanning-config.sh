...
# tuned for this Go Terraform provider template. Add monorepo/e2e paths via CODEQL_PATHS_IGNORE or by
...
if [[ ${_scan} == "1" || ${_scan} == "true" ]]; then
	...
	add_path "**/.venv"
	[[ -d "${REPO}/vendor" ]] && add_path "vendor"
	[[ -d "${REPO}/.terraform" ]] && add_path ".terraform"
	[[ -d "${REPO}/bin" ]] && add_path "bin"
	[[ -d "${REPO}/dist" ]] && add_path "dist"
	[[ -d "${REPO}/modules-dev" ]] && add_path "modules-dev"
fi
