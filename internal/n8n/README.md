...
- **`Client.HTTP`**: standard `*http.Client` whose transport applies, in order: **token-bucket rate limit** (`requests_per_second`), **max concurrent in-flight requests** (`max_concurrent_requests`), then **`X-N8N-API-KEY`** using the provider `api_key`.
