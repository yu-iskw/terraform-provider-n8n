...
page_title: "n8n_credential_notion_api Resource - terraform-provider-n8n"
subcategory: ""
description: |-
...
# n8n_credential_notion_api (Resource)
...
```terraform
resource "n8n_credential_notion_api" "example" {
  name              = "notion"
  api_key           = "placeholder"
  data_version      = 1
...
- `api_key` (String, Sensitive, [Write-only](https://developer.hashicorp.com/terraform/language/resources/ephemeral#write-only-arguments)) Notion Internal Integration Secret. Write-only; never stored in state.
...
- `type` (String) n8n credential type (`notionApi`).
