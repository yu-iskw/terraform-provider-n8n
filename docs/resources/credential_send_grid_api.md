...
page_title: "n8n_credential_send_grid_api Resource - terraform-provider-n8n"
subcategory: ""
description: |-
  Manages an n8n SendGrid API credential (sendGridApi) via the Public API.
  api_key is write-only. Bump data_version to rotate secrets. Terraform 1.11 or later is required.
---

# n8n_credential_send_grid_api (Resource)
...
```terraform
resource "n8n_credential_send_grid_api" "example" {
  name              = "sendgrid"
  api_key           = "placeholder"
  data_version      = 1
...
- `api_key` (String, Sensitive, [Write-only](https://developer.hashicorp.com/terraform/language/resources/ephemeral#write-only-arguments)) SendGrid API key. Write-only; never stored in state.
...
- `type` (String) n8n credential type (`sendGridApi`).
...
```shell
#!/bin/sh
terraform import n8n_credential_send_grid_api.example <credential-id>
```
