...
page_title: "n8n_credential_jira_software_cloud_api Resource - terraform-provider-n8n"
subcategory: ""
description: |-
...
# n8n_credential_jira_software_cloud_api (Resource)
...
- `api_token` (String, Sensitive, [Write-only](https://developer.hashicorp.com/terraform/language/resources/ephemeral#write-only-arguments)) Atlassian API token. Write-only; never stored in state.
...
- `domain` (String) Jira Cloud domain, for example `https://example.atlassian.net`.
...
- `type` (String) n8n credential type (`jiraSoftwareCloudApi`).
...
```shell
#!/bin/bash
# terraform import n8n_credential_jira_software_cloud_api.example {credential_id}
terraform import n8n_credential_jira_software_cloud_api.example REPLACE_WITH_CREDENTIAL_ID
```
