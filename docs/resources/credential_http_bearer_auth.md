...
page_title: "n8n_credential_http_bearer_auth Resource - terraform-provider-n8n"
subcategory: ""
description: |-
...
# n8n_credential_http_bearer_auth (Resource)
...
- `token` (String, Sensitive, [Write-only](https://developer.hashicorp.com/terraform/language/resources/ephemeral#write-only-arguments)) Bearer token. Write-only; never stored in state.
...
- `type` (String) n8n credential type (`httpBearerAuth`).
...
```shell
#!/bin/bash
# terraform import n8n_credential_http_bearer_auth.example {credential_id}
terraform import n8n_credential_http_bearer_auth.example REPLACE_WITH_CREDENTIAL_ID
```
