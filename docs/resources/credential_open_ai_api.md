...
page_title: "n8n_credential_open_ai_api Resource - terraform-provider-n8n"
subcategory: ""
description: |-
...
# n8n_credential_open_ai_api (Resource)
...
```terraform
resource "n8n_credential_open_ai_api" "example" {
  name              = "openai"
  api_key           = "placeholder"
  data_version      = 1
...
- `api_key` (String, Sensitive, [Write-only](https://developer.hashicorp.com/terraform/language/resources/ephemeral#write-only-arguments)) OpenAI API key. Write-only; never stored in state.
...
- `type` (String) n8n credential type (`openAiApi`).
...
```shell
#!/bin/bash
# terraform import n8n_credential_open_ai_api.example {credential_id}
terraform import n8n_credential_open_ai_api.example REPLACE_WITH_CREDENTIAL_ID
```
