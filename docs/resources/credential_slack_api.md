...
page_title: "n8n_credential_slack_api Resource - terraform-provider-n8n"
subcategory: ""
description: |-
...
# n8n_credential_slack_api (Resource)
...
- `access_token` (String, Sensitive, [Write-only](https://developer.hashicorp.com/terraform/language/resources/ephemeral#write-only-arguments)) Slack bot access token. Write-only; never stored in state.
...
- `signature_secret` (String, Sensitive, [Write-only](https://developer.hashicorp.com/terraform/language/resources/ephemeral#write-only-arguments)) Slack signing secret used to verify requests. Write-only; never stored in state.
...
- `type` (String) n8n credential type (`slackApi`).
...
```shell
#!/bin/bash
# terraform import n8n_credential_slack_api.example {credential_id}
terraform import n8n_credential_slack_api.example REPLACE_WITH_CREDENTIAL_ID
```
