...
page_title: "n8n_credential_google_palm_api Resource - terraform-provider-n8n"
subcategory: ""
description: |-
...
# n8n_credential_google_palm_api (Resource)
...
```terraform
resource "n8n_credential_google_palm_api" "example" {
  name              = "gemini"
  api_key           = "placeholder"
  data_version      = 1
...
- `api_key` (String, Sensitive, [Write-only](https://developer.hashicorp.com/terraform/language/resources/ephemeral#write-only-arguments)) Google Gemini API key. Write-only; never stored in state.
...
- `host` (String) Gemini/PaLM API host. n8n defaults to `https://generativelanguage.googleapis.com` when omitted.
...
- `type` (String) n8n credential type (`googlePalmApi`).
...
```shell
#!/bin/bash
# terraform import n8n_credential_google_palm_api.example {credential_id}
terraform import n8n_credential_google_palm_api.example REPLACE_WITH_CREDENTIAL_ID
```
