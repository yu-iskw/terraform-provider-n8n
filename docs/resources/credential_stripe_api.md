...
page_title: "n8n_credential_stripe_api Resource - terraform-provider-n8n"
subcategory: ""
description: |-
...
# n8n_credential_stripe_api (Resource)
...
- `secret_key` (String, Sensitive, [Write-only](https://developer.hashicorp.com/terraform/language/resources/ephemeral#write-only-arguments)) Stripe secret key (`sk_live_` or `sk_test_`). Write-only; never stored in state.
...
- `signature_secret` (String, Sensitive, [Write-only](https://developer.hashicorp.com/terraform/language/resources/ephemeral#write-only-arguments)) Stripe webhook signing secret (`whsec_`). Write-only; never stored in state.
...
- `type` (String) n8n credential type (`stripeApi`).
