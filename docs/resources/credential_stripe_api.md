...
- `secret_key` (String, Sensitive) Stripe secret key (`sk_live_` or `sk_test_`). Write-only; never stored in state.
...
- `signature_secret` (String, Sensitive) Stripe webhook signing secret (`whsec_`). Write-only; never stored in state.
