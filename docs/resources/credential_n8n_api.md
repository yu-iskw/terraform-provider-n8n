...
page_title: "n8n_credential_n8n_api Resource - terraform-provider-n8n"
subcategory: ""
description: |-
...
# n8n_credential_n8n_api (Resource)
...
- `api_key` (String, Sensitive, [Write-only](https://developer.hashicorp.com/terraform/language/resources/ephemeral#write-only-arguments)) n8n API key. Write-only; never stored in state.
- `base_url` (String) n8n Public API base URL, for example `https://example.app.n8n.cloud/api/v1`.
...
- `type` (String) n8n credential type (`n8nApi`).
...
```shell
#!/bin/bash
# terraform import n8n_credential_n8n_api.example {credential_id}
terraform import n8n_credential_n8n_api.example REPLACE_WITH_CREDENTIAL_ID
```
